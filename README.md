...
The immediate integration check after that eigensolver handoff is a very small H2 triplet mainline smoke test under the frozen A-grid local-only route. The goal there is only to verify that the new JAX-native eigensolver can still serve the existing SCF mainline; SciPy remains fallback / audit only, and nonlocal is still not part of this A-grid path. The current follow-up stays just as narrow: one H2 triplet end-to-end micro-profile is used to pin down which SCF bucket dominates after the JAX-native eigensolver reintegration, without reopening singlet work or adding new route families. The present in-loop split shows that this dominant eigensolver bucket is itself led by JAX Hamiltonian application, with weighted orthogonalization second and Rayleigh-Ritz negligible on the current triplet route.
