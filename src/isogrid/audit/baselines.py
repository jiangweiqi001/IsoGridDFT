...
H2_JAX_TRIPLET_END_TO_END_MICRO_PROFILE_BASELINE = H2JaxTripletEndToEndMicroProfileBaseline(
    ...
    converged=True,
    final_total_energy_ha=-1.221447195061,
    final_lowest_eigenvalue_ha=-0.416961171953,
    final_density_residual=0.004575763831706,
    final_energy_change_ha=5.031915031317169e-07,
    total_wall_time_seconds=1338.209,
    average_iteration_wall_time_seconds=74.345,
    dominant_timing_bucket="eigensolver",
    dominant_timing_bucket_fraction_of_total=0.953,
    eigensolver_fraction_of_total=0.953,
    eigensolver_total_wall_time_seconds=1275.896,
    eigensolver_internal_buckets=H2JaxTripletEigensolverInternalBucketBaseline(
        subspace_iteration_wall_time_seconds=1256.493,
        orthogonalization_wall_time_seconds=254.334,
        residual_expansion_wall_time_seconds=62.147,
        rayleigh_ritz_wall_time_seconds=5.378,
        hamiltonian_apply_wall_time_seconds=739.45,
        projected_matrix_build_wall_time_seconds=125.108,
        dominant_internal_bucket="hamiltonian_apply",
    ),
    step1=H2JaxTripletMicroProfileStepBaseline(
        step_index=1,
        solver_backend="jax",
        total_step_wall_time_seconds=110.952,
        eigensolver_wall_time_seconds=107.607,
        static_local_prepare_wall_time_seconds=3.27,
        hartree_solve_wall_time_seconds=2.783,
        energy_eval_wall_time_seconds=1.767,
        density_residual=0.2078899719601,
        energy_change_ha=None,
        eigensolver_subspace_iteration_wall_time_seconds=105.938,
        eigensolver_orthogonalization_wall_time_seconds=20.457,
        eigensolver_residual_expansion_wall_time_seconds=4.982,
        eigensolver_rayleigh_ritz_wall_time_seconds=0.387,
        eigensolver_hamiltonian_apply_wall_time_seconds=63.065,
        eigensolver_projected_matrix_build_wall_time_seconds=10.74,
    ),
    step2=H2JaxTripletMicroProfileStepBaseline(
        step_index=2,
        solver_backend="jax",
        total_step_wall_time_seconds=70.275,
        eigensolver_wall_time_seconds=67.031,
        static_local_prepare_wall_time_seconds=3.179,
        hartree_solve_wall_time_seconds=2.724,
        energy_eval_wall_time_seconds=1.537,
        density_residual=0.1690673203044,
        energy_change_ha=0.0002520342410567977,
        eigensolver_subspace_iteration_wall_time_seconds=65.946,
        eigensolver_orthogonalization_wall_time_seconds=13.009,
        eigensolver_residual_expansion_wall_time_seconds=2.953,
        eigensolver_rayleigh_ritz_wall_time_seconds=0.294,
        eigensolver_hamiltonian_apply_wall_time_seconds=39.576,
        eigensolver_projected_matrix_build_wall_time_seconds=6.452,
    ),
    last_step=H2JaxTripletMicroProfileStepBaseline(
        step_index=18,
        solver_backend="jax",
        total_step_wall_time_seconds=71.121,
        eigensolver_wall_time_seconds=67.995,
        static_local_prepare_wall_time_seconds=3.062,
        hartree_solve_wall_time_seconds=2.63,
        energy_eval_wall_time_seconds=1.575,
        density_residual=0.004575763831706,
        energy_change_ha=5.031915031317169e-07,
        eigensolver_subspace_iteration_wall_time_seconds=66.965,
        eigensolver_orthogonalization_wall_time_seconds=13.459,
        eigensolver_residual_expansion_wall_time_seconds=3.397,
        eigensolver_rayleigh_ritz_wall_time_seconds=0.288,
        eigensolver_hamiltonian_apply_wall_time_seconds=39.514,
        eigensolver_projected_matrix_build_wall_time_seconds=6.702,
    ),
    diagnosis=(
        "This baseline freezes one single post-optimization triplet micro-profile after the JAX-native "
        "fixed-potential eigensolver hot path was further tightened around weighted orthogonalization. "
        "The route still converges in 18 iterations and uses solver_backend='jax' throughout, so the "
        "optimized path remains numerically healthy. Relative to the previous hot-path baseline, total "
        "wall time drops again from about 2427 s to about 1338 s and eigensolver wall time drops from "
        "about 2354 s to about 1276 s. The direct target of this pass, orthogonalization, falls from "
        "about 1009 s to about 254 s, while Hamiltonian application also improves materially and stays "
        "the dominant internal bucket."
    ),
    note=(
        "Triplet end-to-end micro-profile baseline on the frozen JAX A-grid local-only mainline after "
        "one focused orthogonalization hot-path optimization pass. Only one route was run. The run "
